...
class GateTests(unittest.TestCase):
    ...
    def proof(self,**kw):
        cfg=json.loads((self.root/'docs/governance/AI_GATE_CONFIG.json').read_text()); mandatory=cfg['mandatory_read_files']; risk=kw.pop('risk_class','R2_LOGIC')
        data={'schema_version':'0.2.1','task_id':'TASK-123','branch':'task/test','base_commit':self.base,
          ...
          'must_not_change':['scientific evidence semantics','private workspace boundary'],'latest_green_version':'main@'+self.base,
          'allowed_paths':['src/**'],'additional_forbidden_paths':[],'approved_context_changes':[],
    ...
    def prepared(self,**proof_kw): self.normal_changes(); proof=self.proof(**proof_kw); self.report(proof); self.commit(); return proof
    ...
    def test_gate_change_r4_with_flag_passes_preflight(self):
        self.normal_changes(); (self.root/'tools/ai_gate/run_test_profile.py').write_text('# changed gate runner\n'); p=self.proof(risk_class='R4_GOVERNANCE_OR_DEPLOY',allowed_paths=['src/**','tools/ai_gate/**'],gate_change=True); self.report(p); self.commit(); self.assertEqual(self.gate(p).returncode,0)
    ...
    def test_immutable_context_change_requires_explicit_approval(self):
        self.normal_changes(); (self.root/'ARCHITECTURE.md').write_text('# Architecture\nApproved R4 change.\n'); p=self.proof(risk_class='R4_GOVERNANCE_OR_DEPLOY',allowed_paths=['src/**','ARCHITECTURE.md'],gate_change=True); self.report(p); self.commit(); self.assertNotEqual(self.gate(p).returncode,0)
    def test_immutable_context_change_with_explicit_approval_passes(self):
        self.normal_changes(); (self.root/'ARCHITECTURE.md').write_text('# Architecture\nApproved R4 change.\n'); p=self.proof(risk_class='R4_GOVERNANCE_OR_DEPLOY',allowed_paths=['src/**','ARCHITECTURE.md'],gate_change=True,approved_context_changes=['ARCHITECTURE.md']); self.report(p); self.commit(); self.assertEqual(self.gate(p).returncode,0)
    def test_approved_context_change_must_actually_change(self):
        p=self.prepared(risk_class='R4_GOVERNANCE_OR_DEPLOY',gate_change=True,approved_context_changes=['ARCHITECTURE.md']); self.assertNotEqual(self.gate(p).returncode,0)
