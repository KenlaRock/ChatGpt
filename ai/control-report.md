...
Task ID: `GATE-LIVE-SCOPE-20260711-02`
Branch: `gate/ai-gate-live-scope-violation-20260711`
...
This controlled negative acceptance test declares one harmless documentation path but deliberately adds root-level `dummy-file.md`, which is absent from the approved scope. The correct result is a fail-closed preflight rejection by the active main-branch AI Gate.
...
Close the pull request without merging and retain the failed workflow run as provenance that the active gate rejected an undeclared path. No file from this branch should be copied to `main`.
